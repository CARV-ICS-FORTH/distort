...
To minimize overhead, the \texttt{distort-agent} bypasses typical storage middleware. 
\begin{itemize}
    \item \textbf{Discovery Mapping:} It probes \texttt{/sys/class/nvme} continuously to calculate NUMA topology and total block bytes, translating this into resilient \texttt{NVMeDevice} Custom Resources.
    \item \textbf{Partition Slicing:} Using the \texttt{os/exec} package, it implements a wrapper over the \texttt{parted} system utility to inject GUID Partition Tables (GPT) and carve primary partitions on demand dynamically based on Kubernetes limits.
    \item \textbf{NVMe-oF Target Orchestration:} Rather than relying on external Python-based automation suites like \texttt{nvmetcli}, the agent creates Subsystems, Namespaces, and RDMA Ports strictly by writing to the Linux Kernel's ConfigFS mount (\texttt{/sys/kernel/config/nvmet}). This guarantees atomic block-level exports directly from the kernel level.
\end{itemize}
Volume teardown employs Kubernetes \textit{Finalizers}, intercepting \texttt{NVMePartition} deletion events to cleanly un-export the Fabric pathway before dropping the raw partition chunks to reclaim space.
...
While a comprehensive performance evaluation is left for future work, we outline a rigorous methodology to assess DISTORT's efficacy. The primary goal of DISTORT is to provide the operational flexibility of cloud-native storage while preserving the near bare-metal performance characteristics of RDMA-backed NVMe.

\subsection{Performance Baselines}
DISTORT should be evaluated against three distinct storage configurations to contextualize its performance profile:
\begin{enumerate}
    \item \textbf{Local Bare-Metal NVMe:} Represents the theoretical maximum achievable throughput and the absolute minimum latency. This serves as the upper-bound baseline.
    \item \textbf{Standard Network Block Device (NBD / iSCSI):} Provides a traditional, non-RDMA network storage baseline over TCP/IP, highlighting the latency benefits of the fabric interconnect.
    \item \textbf{Commercial/SDS CSI Solutions (e.g., Ceph RBD, OpenEBS):} Represents popular software-defined storage deployments in Kubernetes, demonstrating the overhead introduced by user-space file systems and complex control planes compared to DISTORT's lightweight native configfs approach.
\end{enumerate}

\subsection{Metrics and Workloads}
The primary evaluation tool will be \texttt{fio} (Flexible I/O Tester), deployed as Kubernetes Pods consuming DISTORT-provisioned volumes. The targeted metrics include:
\begin{itemize}
    \item \textbf{Throughput (IOPS \& Bandwidth):} Measured using varying block sizes (e.g., 4KB for IOPS, 1MB for bandwidth) under highly concurrent random read/write workloads.
    \item \textbf{Latency Distributions:} Analyzing 99th and 99.9th percentile latencies critical for performance-sensitive applications like distributed databases.
    \item \textbf{Control Plane Overhead:} Measuring the end-to-end time required to dynamically provision and attach a volume (from PVC creation to Pod `Running` state) to ensure the CRD-driven orchestration scales effectively under cluster churn.
\end{itemize}
