...
func (ns *NodeServer) validateNodeStageRequest(req *csi.NodeStageVolumeRequest) (validatedNodeStageRequest, error) {
	...
	targetBackend := strings.TrimSpace(volumeContext["targetBackend"])
	if targetBackend == "" {
	...
	if targetBackend != spdkTargetBackend && targetBackend != kernelTargetBackend && targetBackend != bxiTargetBackend {
		return validatedNodeStageRequest{}, status.Errorf(codes.InvalidArgument, "Volume context targetBackend %q is invalid", targetBackend)
	}
...
func (ns *NodeServer) connect(ctx context.Context, nqn, ip, port, hostNQN, targetBackend string) (bool, error) {
	if ns.connectRDMA != nil {
		return ns.connectRDMA(ctx, nqn, ip, port, hostNQN)
	}
	if targetBackend == bxiTargetBackend {
		return ConnectBXI(ctx, nqn, ip, port, hostNQN)
	}
...
func (ns *NodeServer) NodeStageVolume(ctx context.Context, req *csi.NodeStageVolumeRequest) (*csi.NodeStageVolumeResponse, error) {
	...
	createdConnection, err := ns.connect(ctx, stage.nqn, stage.portalIP, stage.portalPort, stage.hostNQN, stage.targetBackend)
	if err != nil {
