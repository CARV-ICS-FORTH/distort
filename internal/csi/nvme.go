...
func ConnectRDMA(ctx context.Context, nqn, portalIP, portalPort, hostNQN string) (bool, error) {
	return connectNVMe(ctx, "rdma", nqn, portalIP, portalPort, hostNQN)
}

// ConnectBXI connects to an NVMe-oF target through the Portals/BXI transport.
func ConnectBXI(ctx context.Context, nqn, portalIP, portalPort, hostNQN string) (bool, error) {
	return connectNVMe(ctx, "portals4", nqn, portalIP, portalPort, hostNQN)
}

func connectNVMe(ctx context.Context, transport, nqn, portalIP, portalPort, hostNQN string) (bool, error) {
	klog.InfoS("Connecting NVMe target", "transport", transport, "portalIP", portalIP, "portalPort", portalPort, "nqn", nqn)
	...
	cmd := exec.CommandContext(ctx, "nvme", "connect", "-t", transport, "-a", portalIP, "-s", portalPort, "-n", nqn, "--hostnqn", hostNQN)
