...
func (c *immediatelyExportingClient) Create(ctx context.Context, obj client.Object, opts ...client.CreateOption) error {
	...
	if stored.Spec.TargetBackend == bxiTargetBackend {
		port, err := storageoptions.BXIPort(stored.Spec.TargetOptions)
		if err != nil {
			return err
		}
		stored.Status.PortalPort = port
	}
	return c.Client.Status().Update(ctx, &stored)
}
...
func TestCreateVolumeAcceptsBXIBackendOptions(t *testing.T) {
	server, k8sClient := newControllerTestServer(t)
	req := validCreateRequest("volume-bxi", "team-a")
	req.Parameters["target-backend"] = bxiTargetBackend
	req.Parameters["bxi-nid"] = "192.168.123.7"
	req.Parameters["portals-pid"] = "11"

	response, err := server.CreateVolume(context.Background(), req)
	if err != nil {
		t.Fatalf("CreateVolume returned error: %v", err)
	}
	if response.Volume.VolumeContext["targetBackend"] != bxiTargetBackend {
		t.Fatalf("BXI backend missing from volume context: %#v", response.Volume.VolumeContext)
	}
	if response.Volume.VolumeContext["portalPort"] != "11" {
		t.Fatalf("BXI port missing from volume context: %#v", response.Volume.VolumeContext)
	}
	var partition storagev1alpha1.NVMePartition
	if err := k8sClient.Get(context.Background(), types.NamespacedName{Name: "volume-bxi", Namespace: "team-a"}, &partition); err != nil {
		t.Fatal(err)
	}
	if partition.Spec.TargetBackend != bxiTargetBackend || partition.Spec.VolumeManager != partitionVolumeManager {
		t.Fatalf("unexpected BXI storage selection: %#v", partition.Spec)
	}
	if partition.Spec.TargetOptions["bxi-nid"] != "192.168.123.7" || partition.Spec.TargetOptions["portals-pid"] != "11" {
		t.Fatalf("BXI options were not persisted: %#v", partition.Spec.TargetOptions)
	}
}
