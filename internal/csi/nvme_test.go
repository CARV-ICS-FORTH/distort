...
func TestConnectBXIUsesPortalsTransportAndPreservesHostAuthorization(t *testing.T) {
	fakeBin := t.TempDir()
	commandLog := filepath.Join(t.TempDir(), "nvme.log")
	script := `#!/usr/bin/env bash
set -eu
...
`
	...
	created, err := ConnectBXI(context.Background(), "nqn.test", "192.168.123.7", "11", "nqn.test:host")
	if err != nil || !created {
		t.Fatalf("ConnectBXI created=%t err=%v, want true, nil", created, err)
	}
	calls, err := os.ReadFile(commandLog)
	if err != nil {
		t.Fatal(err)
	}
	want := "list-subsys -o json\nconnect -t portals4 -a 192.168.123.7 -s 11 -n nqn.test --hostnqn nqn.test:host"
	if got := strings.TrimSpace(string(calls)); got != want {
		t.Fatalf("nvme calls = %q, want %q", got, want)
	}
}
