...
func TestRoundUp(t *testing.T) {
	tests := []struct {
		...
		{name: "one allocation unit", input: AllocationUnitBytes, want: AllocationUnitBytes, ok: true},
		{name: "one allocation unit plus one", input: AllocationUnitBytes + 1, want: 2 * AllocationUnitBytes, ok: true},
