...
func TestBuiltInPluginsAreRegistered(t *testing.T) {
	for _, name := range []string{"spdk", "kernel", "bxi"} {
...
func TestBXIRequiresMatchingRunningPortalsPID(t *testing.T) {
...
case "$1" in
  ...
  *) exit 0 ;;
	...
	inspectBXIPortalPID = func() (string, bool, error) { return "11", true, nil }
	...
	if err := EnsureBXISPDKRunning(context.Background(), "0x1", 11); err != nil {
		t.Fatalf("matching BXI process was rejected: %v", err)
	}
	if err := EnsureBXISPDKRunning(context.Background(), "0x1", 12); err == nil || !strings.Contains(err.Error(), "PORTALS_PID") {
		t.Fatalf("mismatched BXI process error = %v, want PORTALS_PID mismatch", err)
	}
}
