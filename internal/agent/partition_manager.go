...
const (
	...
	bxiTargetBackend  = "bxi"
)

func usesSPDKStorage(targetBackend string) bool {
	return targetBackend == spdkTargetBackend || targetBackend == bxiTargetBackend
}
...
func resolvePartitionPlugins(partition *storagev1alpha1.NVMePartition) (resolvedPartitionPlugins, string, error) {
	...
	if volumeManagerName == "" || volumeManagerName == "partition" {
		if usesSPDKStorage(targetBackendName) {
...
func (p *PartitionManager) provisionPartition(
	...
	portalPort := 4420
	if resolved.targetBackendName == bxiTargetBackend {
